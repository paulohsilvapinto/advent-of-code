...
def get_adjacent_points(engine_schematic_matrix, start_position, end_position):
    ...
    # print(end_position)
    adjacent_positions = []
    for row_search_idx in range(max(0, start_position.row - 1), min(len(engine_schematic_matrix), end_position.row + 2)):
        for col_search_idx in range(max(0, start_position.column - 1), min(len(engine_schematic_matrix[0]), end_position.column + 2)):
    ...
    for col_idx, col_val in enumerate(engine_schematic_matrix[row_idx]):
        if col_val.isnumeric():
            ...
            if number_builder:
                ...
                number_builder = ""
                number_adjacencies_list.append(NumberAdjacencies(current_number, get_adjacent_points(engine_schematic_matrix, start_position, end_position)))
