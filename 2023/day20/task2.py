...
def find_min_presses_for_signal(circuit, target_module="rx", target_signal=Signal.LOW):
    modules_before_target = list(circuit[target_module].input_signals.keys())
    modules_two_levels_from_target = [
        module_name
        for module_name, module in circuit.items()
        for before_rx in modules_before_target
        if before_rx in module.output_modules
    ]

    if all(
        isinstance(circuit[module], ConjunctionModule)
        for module in modules_two_levels_from_target + modules_before_target
    ):
        expected = {
            module: {"expected_signal": ~target_signal, "min_presses": INFINITY}
            for module in modules_two_levels_from_target
        }
    else:
        raise NotImplementedError
        ...
        while execution_queue:
            ...
            if output_signal is not None:
                for output_module in circuit[current_module].output_modules:
                    if expected.get(current_module, {}).get("expected_signal") == output_signal:
                        expected[current_module]["min_presses"] = min(
                            expected[current_module]["min_presses"],
                            button_presses,
                        )

                        if all([val["min_presses"] != INFINITY for val in expected.values()]):
                            return lcm(*[val["min_presses"] for val in expected.values()])
...
def solve(input_data):
    ...
    return find_min_presses_for_signal(circuit)
